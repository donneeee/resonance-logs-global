...
const outputPath = path.join(root, "DEV_exports/stasis-factor-grade-descriptions.csv");

const data = JSON.parse(fs.readFileSync(inputPath, "utf8"));

function isStasisFactor(row) {
  const names = [
    row.familyName,
    ...Object.values(row.familyNames ?? {}),
  ].filter(Boolean).join(" ");
  return /\bStasis\b|稳态|穩態|Stase|Estasis|Estase/i.test(names);
}
...
function classOf(name) {
  ...
  if (/^Stasis\s+X\s*\d+\b/i.test(text)) return "All Classes";

  const match = text.match(/^(.*?)\s+Stasis\s+X\s*\d+\b/i);
  return match?.[1]?.trim() || "";
}
