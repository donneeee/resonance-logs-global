...
pub(crate) fn build_minimap_snapshot(state: &AppState) -> MinimapSnapshot {
    ...
    for (&uuid, entity) in &state.encounter.entity_uuid_to_entity {
        ...
        let has_mechanic_buff = has_minimap_mechanic_buff(state, uuid, config);
        if !is_minimap_relevant(
            uuid,
            local_uuid,
            &state.team,
            monster_id,
        ...
        let kind = classify(uuid, local_uuid, entity_type, monster_id, has_mechanic_buff);
    ...
    let markers = state
        ...
        .markers
        .values()
        .map(|marker| MinimapMarker {
            marker: marker.skill_id - 1100,
...
fn is_minimap_relevant(
    ...
    has_mechanic_buff: bool,
) -> bool {
    if uuid == local_uuid || team.members.contains(&uuid) || has_mechanic_buff {
        return true;
    }
    matches!(
        (monster_id, config),
...
fn classify(
    ...
    match entity_type {
        ...
        EEntityType::EntDummy if has_mechanic_buff => MinimapEntityKind::Monster,
        EEntityType::EntDummy => MinimapEntityKind::Dummy,
        _ if has_mechanic_buff => MinimapEntityKind::Monster,
